...
function parseDashscopeContent(response) {
  ...
  if (Array.isArray(content)) {
    const textPart = content.find((part) => part && typeof part.text === 'string' && part.text.trim().length > 0);
    if (textPart) {
...
function getProvider() {
  ...
  if (!SUPPORTED_PROVIDERS.has(provider)) {
    throw new Error(`Unsupported AI summaries provider "${provider}".`);
  }
  return provider;
}

export async function enrichReportWithAiSummaries(report, { logger = console } = {}) {
  if (!isSummariesEnabled()) {
    return report;
  }
  ...
  if (!getSummaryApiKey()) {
    logger.warn?.(`[ai] Summaries enabled but apiKey is not configured for provider "${provider}".`);
    return report;
  }
  ...
  let promptConfig;
  try {
    promptConfig = loadSummaryPromptConfig();
  } catch (error) {
  ...
  if (typeof systemPrompt !== 'string' || systemPrompt.trim().length === 0) {
    const error =
      'Summary prompt config is missing systemPrompt.';
  ...
  if (typeof template !== 'string' || template.trim().length === 0) {
    const error =
      'Summary prompt config is missing userPromptTemplate.';
    logger.error?.(`[ai] ${error}`);
    report.aiSummary = {
      status: 'error',
      error
  ...
  try {
    if (provider === 'dashscope') {
      apiResponse = await createDashscopeChatCompletion(requestPayload);
    } else {
