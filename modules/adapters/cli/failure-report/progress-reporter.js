import { createProgressReporter } from '../../../core/failure-report/progress.js';
...
export function createCliProgressReporter({ stream = process.stdout } = {}) {
  ...
  return createProgressReporter({
    ...
    stepComplete: ({ jobId, jobName, stepNumber, durationMs, contextCount }) => {
      const jobLabel = jobName || `job ${jobId}`;
      const contextsInfo = contextCount != null ? `, contexts: ${contextCount}` : '';
      write(
        withPrefix(
          '  ✓ ',
    ...
    stepLogFetchStart: ({ jobId, jobName, stepNumber, stepName }) => {
      ...
      write(
        withPrefix(
          '    ⇢ ',
          `[${jobLabel}] Fetching log for step ${stepNumber}${stepName ? ` (${stepName})` : ''}`
        )
      );
    },
